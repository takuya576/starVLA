...
def prepare_data(
    ...
    vlm_train_dataloader = build_dataloader(
        cfg=cfg, dataset_py=cfg.datasets.vlm_data.dataset_py
    )
...
class VLAMTrainer(TrainerUtils):
    ...
    def prepare_training(self):
        ...
        if (
            hasattr(self.config.trainer, "pretrained_checkpoint")
            and self.config.trainer.pretrained_checkpoint
        ):
            pretrained_checkpoint = self.config.trainer.pretrained_checkpoint
            reload_modules = (
                self.config.trainer.reload_modules
                if hasattr(self.config.trainer, "reload_modules")
                else None
            )
    ...
    def _save_initial_configs(self):
        ...
        if isinstance(self.config, AccessTrackedConfig):
            ...
            logger.info(
                f"\U0001f4ca Accessed config snapshot saved at {output_dir / 'config.yaml'}"
            )
    ...
    def _init_wandb(self):
        ...
        if os.environ.get("WANDB_MODE") == "disabled" or os.environ.get(
            "WANDB_DISABLED", ""
        ).lower() in {
            "1",
            "true",
        ...
        if self.accelerator.is_main_process:
            try:
                ...
                logger.warning(
                    f"W&B init failed; continuing without W&B: {exc}"
                )
                self._wandb_enabled = False
    ...
    def train(self):
        ...
        while self.completed_steps < self.config.trainer.max_train_steps:
            ...
            if (
                optimizer_stepped
                and self.completed_steps % self.config.trainer.eval_interval
                == 0
            ):
            ...
            if (
                optimizer_stepped
                and self.completed_steps % self.config.trainer.save_interval
                == 0
    ...
    def eval_action_model(self, step_metrics: dict = None) -> float:
        ...
        if self.accelerator.is_main_process:
            ...
            output_dict = self.accelerator.unwrap_model(
                self.model
            ).predict_action(examples=examples)
            normalized_actions = output_dict["normalized_actions"]

            actions = np.array(actions)
            num_pots = np.prod(actions.shape)
            score = TrainerUtils.euclidean_distance(
    ...
    def _finalize_training(self):
        ...
        if self.accelerator.is_main_process and getattr(
            self, "_wandb_enabled", False
        ):
            try:
                wandb.finish()
